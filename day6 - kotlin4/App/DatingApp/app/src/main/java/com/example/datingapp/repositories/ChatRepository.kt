...
import com.example.datingapp.models.ChatRoom
...
class ChatRepository {
    ...
    fun getMessages(chatRoomId: String, onResult: (List<Message>) -> Unit) {
        ...
        firestore.collection("chatRooms")
            .document(chatRoomId)
            .collection("messages")
            .orderBy("timestamp") // 메시지를 시간 순서로 정렬
            .get()
            .addOnSuccessListener { querySnapshot ->
                val messages = querySnapshot.documents.mapNotNull { document ->
                    document.toObject(Message::class.java)
                }
                onResult(messages) // 결과 콜백으로 메시지 리스트 반환
            }
            .addOnFailureListener { exception ->
                // 오류 처리
                Log.w("ChatRepository", "Error getting messages: ", exception)
                onResult(emptyList()) // 오류 시 빈 리스트 반환
            }
    }
